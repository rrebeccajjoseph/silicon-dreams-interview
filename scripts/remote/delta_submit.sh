#!/bin/bash
# Submit the training matrix to Delta as dependent Slurm jobs, each on a whole
# gpuA100x4 node (64 cores -> 64 env worker processes). Stages are time-boxed with
# --minutes so the chain finishes on a schedule regardless of throughput.
...
COMMON="--envs $ENVS --workers 64 --iters 100000 --out $CK $WB"
...
BN=$(job mono_b_norelabel "" $(mins 110) "$PY mono-b --no-relabel --minutes $(mins 100) --envs $ENVS --workers 64 --iters 100000 --out $CK/ablation --run-name mono_b_norelabel $WB")
