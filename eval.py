"""The single evaluation entry point: loads the trained checkpoint and runs the deployed system on
the full task (pick, lift, delayed target, in-hand reorientation, 2 s hold).

  python eval.py                         # submitted system: scripted grasp + learned reorient
  python eval.py hold                    # same grasp, then hold still (baseline)
  python eval.py s --episodes 200 --seed 123 --name s_policy
  python eval.py s --grid --wide         # sweep the wide r x h envelope, both start poses
  python eval.py --merge a b --name ab   # combine results from parallel runs
  python eval.py a|b|c|random ...        # the other plans in the repo (not trained to convergence)

Writes results/<name>.json with every episode (geometry, start pose, termination reason, stage,
final errors, peak joint speeds against the URDF limits) and a summary."""
...
from inhand.scene import ARM_VELOCITY, HAND_VELOCITY, Scene  # noqa: E402
...
def build(args, scene, cfg) -> System:
    ...
    if args.plan == "s":
        return PlanS(scene, cfg, args.reorient or str(ROOT / "checkpoints" / "reorient_deploy.pt"))
    if args.plan == "hold":
        return PlanS(scene, cfg, None)
...
def run_episode(env: FullTaskEnv, system: System, obj: ObjectParams | None, pose: str | None) -> dict:
    ...
    system.reset()
    info = {}
    arm_speed = hand_speed = 0.0
    hand_over = hand_over_saturated = 0
    effort = np.abs(env.m.actuator_forcerange[env.ids.hand_acts, 1])
    while True:
        obs, _, done, info = env.step(system.act(obs["deploy"]))
        arm_speed = max(arm_speed, float(np.abs(env.d.qvel[env.ids.arm_dofs]).max()))
        v = np.abs(env.d.qvel[env.ids.hand_dofs])
        hand_speed = max(hand_speed, float(v.max()))
        over = v > HAND_VELOCITY
        if over.any():
            # at the torque limit the joint is being back-driven by the object, not driven by the command
            hand_over += 1
            hand_over_saturated += int((np.abs(env.d.actuator_force[env.ids.hand_acts][over]) >= 0.99 * effort[over]).all())
        if done:
            break
    ...
    return {
        ...
        # sampled at the control rate; torque limits are enforced by the actuators themselves
        "max_arm_speed": arm_speed, "max_hand_speed": hand_speed,
        "hand_steps_over_limit": hand_over, "hand_steps_over_limit_saturated": hand_over_saturated,
    }
...
def report(rows: list[dict]) -> dict:
    ...
    out["envelope"] = env
    out["speed_limits"] = {
        "arm_limit": ARM_VELOCITY, "hand_limit": HAND_VELOCITY,
        "episodes_over_arm_limit": float(np.mean([r["max_arm_speed"] > ARM_VELOCITY for r in rows])),
        "episodes_over_hand_limit": float(np.mean([r["max_hand_speed"] > HAND_VELOCITY for r in rows])),
        "peak_arm": float(max(r["max_arm_speed"] for r in rows)), "peak_hand": float(max(r["max_hand_speed"] for r in rows)),
        "hand_control_steps_over_limit": float(np.sum([r.get("hand_steps_over_limit", 0) for r in rows]) / np.sum([r["steps"] for r in rows])),
        "of_which_at_torque_limit": float(np.sum([r.get("hand_steps_over_limit_saturated", 0) for r in rows]) / max(1, np.sum([r.get("hand_steps_over_limit", 0) for r in rows]))),
    }
    return out
...
def main():
    ...
    p.add_argument("--grasp"); p.add_argument("--policy")
    p.add_argument("--reorient", help="plan s checkpoint (default checkpoints/reorient_deploy.pt)")
